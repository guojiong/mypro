...
class InStoreForm(forms.Form):
    ...
    mtype = forms.CharField(label='材料分类', max_length=128, strip=True, widget=forms.Select(attrs={'id': 'mtype', 'class': 'form-control', 'style': "width:180px", 'value': ''}))
    mclass = forms.ModelChoiceField(queryset=Mclass.objects.values_list('mclass').order_by('mclass').distinct(), empty_label='', label='材料小类', widget=forms.Select(attrs={'id': 'mclass', 'class': 'form-control', 'style': "width:180px", 'value': ''}))
    ...
    price = forms.CharField(label='税后单价', max_length=128, strip=True, widget=forms.TextInput(attrs={'id': 'price', 'type': 'number', 'min': "0", 'class': 'form-control', 'style': "width:180px", 'value': '', 'readonly': 'true'}))
